...
const ToDoCard = () => {
  ...
  return (
    <div className="border border-[#E4E4E4] min-w-[288px] rounded-lg py-[11px] px-7">
      <h2 className="text-gray-800 font-semibold text-[15px] mb-[10px]">To-Do List</h2>
      ...
      <div className="mb-4 flex">
        ...
        />
        <button onClick={addTask} className="bg-[#FF4B00] text-white px-4 rounded-r-lg">
          Add
        </button>
      </div>
      ...
      <ul className="space-y-3 h-[170px] overflow-y-auto">
        {tasks.map((task) => (
          <li key={task.id} className="items-center justify-between rounded-lg transition">
            <div className="flex items-center cursor-pointer" onClick={() => toggleTask(task.id)}>
              ...
              <span className={`text-sm text-black ${task.completed ? "line-through" : ""}`}>
                {task.text}
              </span>
            </div>
