...
// Define JWT settings
const JWT_SECRET: string = "123qweasdzxc";
const JWT_EXPIRES_IN: string | number = "1h"; // Ensure it's a valid type
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const token = jwt.sign(
      { userId: user.id },
      JWT_SECRET,
      { expiresIn: JWT_EXPIRES_IN } as jwt.SignOptions // Explicitly type the options object
    );
